...
RasterizerService.prototype.startService = function() {
  var rasterizer = spawn(this.config.command, [path.join(__dirname, '../scripts/rasterizer.js'), this.config.path, this.config.host + ':' + this.config.port, this.config.viewport]);
  ...
  rasterizer.on('exit', this.rasterizerExitHandler.bind(this));
